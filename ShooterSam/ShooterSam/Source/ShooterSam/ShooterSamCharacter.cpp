...
#define LOG(x, y) UE_LOG(LogTemp, Display, TEXT(x), y)
...
void AShooterSamCharacter::BeginPlay()
{
	...
	if (SpawnedGun)
	{
		...
		SpawnedGun->OwnerController = GetController();
	}
                                         ...
                                         class AController* InstigatedBy, AActor* DamageCauser)
{
	if (IsAlive)
	{
		...
		if (Health <= 0.0f)
		{
			...
			LOG("Character is dead: %s", *GetActorNameOrLabel()) // UE_LOG(LogTemp, Display, TEXT("Character is dead: %s"), *GetActorNameOrLabel());
		}
	}
