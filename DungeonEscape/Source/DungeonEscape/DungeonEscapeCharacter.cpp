...
void ADungeonEscapeCharacter::Interact() // When using Left mouse button or E key for interact.
{
	FVector Start = GetFirstPersonCameraComponent()->GetComponentLocation();
	FVector End = Start + (FirstPersonCameraComponent->GetForwardVector() * MaxInteractionDistance);
	// DrawDebugLine(GetWorld(), Start, End, FColor::Red, false, 5.0f);

	FCollisionShape InteractionSphere = FCollisionShape::MakeSphere(InteractionSphereRadius); // Make a Sphere CollisionShape.
	...
	if (HasHit)
	{
		...
		if (HitActor->ActorHasTag("CollectableItem")) // Look if actor has a tag named "CollectableItem" or not
		{
			// HitActor is a Collectable item.
			ACollectableItem* CollectableItem = Cast<ACollectableItem>(HitActor);
			if (CollectableItem)
			{
				ItemList.Add(CollectableItem->ItemName); // Add item to the ItemList and destroy it.
				CollectableItem->Destroy();
			}
		}
		else if (HitActor->ActorHasTag("Lock")) // Look if actor has a tag named "Lock" or not
		{
			...
			{
				// 1 - Is the lock empty?
				if (!Lock->GetIsKeyPlaced()) // Lock is empty.
				{                                                                  // 2 - Do we have a KeyItemName in our ItemList? 
					int32 ItemsRemoved = ItemList.RemoveSingle(Lock->KeyItemName); // 3 - Remove the item from our Inventory if we have it
					if (ItemsRemoved)
					{
						Lock->SetIsKeyPlaced(true); // 4 - Activate the lock
				...
				else // Lock has a key inside.
				{
					ItemList.Add(Lock->KeyItemName); // Add the KeyItemName into ItemList
					Lock->SetIsKeyPlaced(false); // Deactivate the lock.
				}
			}
		}
	}
